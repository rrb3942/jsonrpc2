...
var (
	...
	// ErrHTTPNotJSON indicates that the HTTP server returned a successful status code (2xx)
	// but the Content-Type header was not "application/json".
	ErrHTTPNotJSON = errors.New("jsonrpc2/httpbridge: response Content-Type was not application/json")
)
...
//     attempts to unmarshal the body into `v`. Returns any unmarshalling error.
...
// [ErrHTTPEmptyResponse], [ErrHTTPNotJSON], and [ErrHTTPResponse].
func (h *HTTPBridge) Decode(ctx context.Context, v any) error {
	...
	if h.respCode >= 200 && h.respCode < 300 {
		...
		return fmt.Errorf("%w (status: %s)", ErrHTTPNotJSON, h.respStatus)
	}
